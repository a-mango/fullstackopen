...
beforeEach(async () => {
  ...
  const newUser = await api.post(helper.userApi).send(helper.mockUser)

  // Authorize the user
  const authorization = await api.post('/login').send({
    username: helper.mockUser.username,
    password: helper.mockUser.password,
  })
